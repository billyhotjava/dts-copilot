...
| ID | Task | 优先级 | 状态 | 依赖 |
|----|------|--------|------|------|
| T01 | 套件骨架定义（六要素清单 + 模板） | P1 | DONE | S31-F5,S32-F1,F2 |
| T02 | 场景脚手架脚本 | P1 | DONE | T01 |
| T03 | 升级 Sprint-30 F5 checklist 为可执行套件 | P1 | DONE | T01 |
...
- **验证**：六要素模板齐全，能映射到现有报花/财务两域。可重跑 `../../it/test_f3_scenario_onboarding_kit.sh`。
...
- **影响**：`scripts/scaffold-scenario-kit.mjs` + 模板引用 T01。
...
- **验证**：F3 已验证手册、模板和脚手架结构；F4 用本手册实际接入一个域。
