...
- **结论(关键风险)**:乐观执行需要的 `assumptions[]`(可改口径)、`confidence`(置信度)、低置信时的 `clarifications[]`(澄清选项)三类数据,**后台 SSE 与同步接口当前均未返回**;改后重算所需的 `assumptionOverrides` 入参也**尚未支持**。T02/T03/T04 已写明需扩展接口契约(前端 type + 后台返回/入参),前端先按契约做「有则用、无则降级到现有 `qualityNotes`/不显示芯片」的容错,后台补齐前不阻塞 UI。
...
- 依赖:**F3**(对话脊柱:消息流 + 发送/流式接入已扶正)、**F4**(活产物画布:产物引用接口 + `useArtifactStore` 已就绪)。
...
| T02 | 乐观执行流(高置信直接出结果 + 假设芯片) | P0 | READY | T01, F3 |
| T03 | 低置信降级反问(澄清芯片单选) | P0 | READY | T02 |
| T04 | 芯片可改 → 重算 → 画布原地刷新 | P0 | READY | T01, T02, F4 |

## 完成标准

- [ ] 高置信问数:一句话直接流式出结果,结果卡顶部常驻可读的口径芯片(如「本月=2026-05 ✎」「利润=收入−成本 ✎」「范围=在租项目 ✎」),消费后台返回的 `assumptions[]`(后台未返回时按容错降级,不报错)(T01、T02)。
- [ ] 低置信问数:置信度低于阈值时不直接猜,渲染澄清芯片(单选,如「本月 = 自然月 | 账期」),用户选定后再带选择发起执行;阈值与降级判定写清且有单测(T03)。
- [ ] 点击口径芯片进入编辑态 → 提交新假设 → 以 `assumptionOverrides` 重新发起查询 → F4 画布对应产物**按同一 `artifactId` 原地刷新**(不新增产物、旧产物不被复制进托盘)(T04)。
