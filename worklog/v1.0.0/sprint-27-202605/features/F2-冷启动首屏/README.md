...
| ID | Task | 优先级 | 状态 | 依赖 |
|----|------|--------|------|------|
| T01 | 首屏布局与大输入框 | P0 | READY | F1 |
| T02 | 语音入口接入 | P0 | READY | T01 |
| T03 | 起手芯片 | P0 | READY | T01 |
| T04 | 三张卡数据接入 | P1 | READY | T01 |

## 完成标准

- [ ] 进入工作台空态时第一眼是冷启动首屏：居中大输入框（占位「问一句，或说一句…」）+ 起手芯片 + 三张卡，不再是旧菜单或空白。
- [ ] 在大输入框输入并提交（回车 / 点发送）即发起会话进入对话脊柱（F3），首屏退场。
- [ ] 语音按钮可用：点按录音、转写文本回填输入框，可直接发送。
- [ ] 起手芯片可点：高频问题芯片点击即发起对应查询；固定报表芯片点击跳转或直接发起对应查询。
- [ ] 三张卡有真实数据：我的资产显示看板/卡片计数；继续上次会话显示最近会话并可点回跳；主动信号卡至少有规则化占位提醒，无信号时优雅降级。
- [ ] 首屏新建组件单文件 < 800 行，关键行为（提交发起、芯片点击、空态降级）有 vitest 测试护栏。
