...
- [x] `/agent-bi` 工作台渲染双栏：左侧脊柱占位约 40% 宽、右侧画布占位填满剩余宽。
- [x] `/auth/*`、`/screens/*`、`/public/*` 全屏/极简渲染，不出现工作台双栏与左导航。
- [x] 治理/资产页（`/data`、`/dashboards`、`/questions`）仍为标准单列布局，正常可用。
- [x] 工作台路由下不再出现浮动 `CopilotSidebar`。
- [x] 窄屏断点下双栏降级为单栏，无横向溢出基础约束。
- [x] `pnpm typecheck` / `pnpm build` 通过。

## 完成标准

- [x] `AppLayout` 区分全屏布局 / 工作台双栏布局 / 标准单列布局三种分支。
- [x] 双栏容器结构与 CSS 落地，F3/F4 插槽就位。
- [x] 鉴权逻辑零回归；公开路由不再挂工作台壳。
