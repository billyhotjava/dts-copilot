...
2. **触发源迁移到画布动作行**：
   - F4 的 `CanvasActions`（活产物画布顶部动作行，spec §3 右侧画布：`[存为卡片][钉到看板][</> SQL·溯源][导出]`）新增/对接「`</>` SQL·溯源」按钮，点击 `onOpenTrace(activeProductMessageId)`。
   - 由画布容器（F4）持有 `traceOpen` 状态与「当前在看产物」对应的 `message`，把 `toolMessages` 等传给 `TracePanel`。
   - **契约约定**（写给 F4）：`CanvasActions` 暴露 `onOpenTrace?: (messageId: string) => void` 回调；画布维护 `activeProduct.messageId`。本 Task 提供 `TracePanel` 受控接口，F4 负责接线；若 F4 尚未落地动作行，T01 先在 `CopilotChat` 内用临时按钮触发面板，并标注 `// TODO(F4): 由 CanvasActions 接管触发`。
...
- F4 `CanvasActions`（动作行）：新增「`</>` SQL·溯源」按钮 + `onOpenTrace` 回调（跨 Feature 契约，F4 接线）。
...
- [ ] `TracePanel` 受控接口（`open`/`onClose`）就绪，与 F4 `CanvasActions` 的事件契约（`onOpenTrace(messageId)`）已写明；F4 未就绪时有临时触发并标注 TODO。
