...
F2 应用 MySQL oracle 证明链路记录：`it/evidence/20260606-local/f2-application-mysql-oracle-proof.md`（PASS；dbt 包保存在 `dts-copilot/worklog/prs/v1`，不放入 `dts-stack`）。
...
| IT-03 | 汇总双路对账 | F2-T02 | `it/test_f2_summary_dual_reconciliation.sh` + `it/test_f2_application_mysql_oracle_proof.sh` + `worklog/prs/v1/tests/test_xycyl_finance_dbt_zip_contract.sh` + `it/evidence/20260606-local/f2-application-mysql-oracle-proof.md` | IN_PROGRESS（summary comparator + case registry + SQL 方言 contract PASS；月对账/售账/2026 凭证 ADS vs 应用 MySQL 原表 oracle SQL contract + 应用内 proof runner/resource + JDBC executor/config PASS；v1 dbt zip 已补售账链和月对账 `projectId`；planner 已防 `prs.finance.voucher.profile` 抢走 TPL-57 ADS 模板；live L2/签字 SQL vs copilot dataset 双路取数待接线） |
