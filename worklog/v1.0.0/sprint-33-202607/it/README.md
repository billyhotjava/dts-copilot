...
F2 应用 MySQL oracle 证明链路记录：`it/evidence/20260606-local/f2-application-mysql-oracle-proof.md`（PASS；dbt 包保存在 `dts-copilot/worklog/prs/v1`，不放入 `dts-stack`；runtime HTTP smoke 使用 `it/test_f2_application_mysql_oracle_runtime_http.sh`，本地端到端直连证明使用 `it/test_f2_application_mysql_oracle_runtime_live_local.sh`，已登记应用 MySQL 证明使用 `it/test_f2_application_mysql_oracle_runtime_registered_datasource.sh`）。
...
| IT-03 | 汇总双路对账 | F2-T02 | `it/test_f2_summary_dual_reconciliation.sh` + `it/test_f2_application_mysql_oracle_proof.sh` + `it/test_f2_application_mysql_oracle_runtime_http.sh` + `it/test_f2_application_mysql_oracle_runtime_live_local.sh` + `it/test_f2_application_mysql_oracle_runtime_registered_datasource.sh` + `worklog/prs/v1/tests/test_xycyl_finance_dbt_zip_contract.sh` + `it/evidence/20260606-local/f2-application-mysql-oracle-proof.md` | IN_PROGRESS（summary comparator + case registry + SQL 方言 contract PASS；月对账/售账/2026 凭证 ADS vs 应用 MySQL 原表 oracle SQL contract + 应用内 proof runner/resource + JDBC executor/config + runtime HTTP smoke PASS；v1 dbt zip 已补售账链和月对账 `projectId`；planner 已防 `prs.finance.voucher.profile` 抢走 TPL-57 ADS 模板；凭证 oracle SQL 已按真实 `YYYYMM` 账期与凭证头口径修正；直连应用 MySQL 本地端到端 proof 当前对已构建的凭证 ADS 返回 `PASSED`，`COPILOT_FINANCE_PROOF_CASE_IDS=all` 已具备全量 seed/proof 与缺 ADS 前置失败；已登记 `ptr_mysql` 通过平台入湖任务源回退返回 `PASSED`，`园林业务库` 保留为非同源反例；live L2/签字 SQL vs copilot dataset 双路取数待接线） |
