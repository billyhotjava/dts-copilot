...
F2 应用 MySQL oracle 证明链路记录：`it/evidence/20260606-local/f2-application-mysql-oracle-proof.md`（PASS；dbt 包保存在 `dts-copilot/worklog/prs/v1`，不放入 `dts-stack`；runtime HTTP smoke 使用 `it/test_f2_application_mysql_oracle_runtime_http.sh`，本地端到端直连证明使用 `it/test_f2_application_mysql_oracle_runtime_live_local.sh`）。
