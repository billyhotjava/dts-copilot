# Sprint-33 F1 live authority auth precheck

**时间**: 2026-06-05
**环境**: local docker compose (`dts-stack-dts-admin-1`, `dts-copilot-analytics`)
**结论**: BLOCKED FOR LIVE。F1 明细对账 harness 的本地 contract 已 PASS，但真实 L2 adminapi 权威基准端点仍需要有效用户/JWT 会话，当前不能用无凭证、服务头或 test token 作为财务权威基准证据。
...
- `APP_TEST_API_TOKEN` / `X-Test-Token` 只适用于 `/test/**` helper endpoints，不能作为 `/rs-flowers-base/...` 财务 L2 权威基准证据。
- 因 live L2 权威基准与 analytics `/api/dataset` 可能需要不同凭证，已按 TDD 增加分离配置:
  - `FINANCE_RECONCILIATION_AUTHORITY_AUTHORIZATION`
  - `FINANCE_RECONCILIATION_AUTHORITY_COOKIE`
  - `FINANCE_RECONCILIATION_ANALYTICS_AUTHORIZATION`
  - `FINANCE_RECONCILIATION_ANALYTICS_COOKIE`
  - 旧 `FINANCE_RECONCILIATION_ORACLE_AUTHORIZATION/COOKIE`、`FINANCE_RECONCILIATION_AUTHORIZATION/COOKIE` 仅作为兼容 fallback。
