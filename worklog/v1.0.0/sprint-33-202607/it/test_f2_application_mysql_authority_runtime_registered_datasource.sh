...
COPILOT_FINANCE_APPLICATION_MYSQL_AUTHORITY_ENABLED=true \
COPILOT_FINANCE_APPLICATION_MYSQL_AUTHORITY_JDBC_URL="$APP_JDBC_URL" \
COPILOT_FINANCE_APPLICATION_MYSQL_AUTHORITY_USERNAME="$APP_USERNAME" \
COPILOT_FINANCE_APPLICATION_MYSQL_AUTHORITY_PASSWORD="$APP_PASSWORD" \
COPILOT_FINANCE_APPLICATION_MYSQL_AUTHORITY_DATABASE=rs_cloud_flower \
COPILOT_FINANCE_APPLICATION_MYSQL_AUTHORITY_COPILOT_JDBC_URL="jdbc:postgresql://${ADS_HOST}:${ADS_PORT}/${ADS_DB}" \
COPILOT_FINANCE_APPLICATION_MYSQL_AUTHORITY_COPILOT_USERNAME="$ADS_USERNAME" \
COPILOT_FINANCE_APPLICATION_MYSQL_AUTHORITY_COPILOT_PASSWORD="$ADS_PASSWORD" \
COPILOT_FINANCE_APPLICATION_MYSQL_AUTHORITY_COPILOT_DATABASE=prs.flowerbiz.federated \
...
COPILOT_FINANCE_PROOF_CASE_IDS="$CASE_IDS_RAW" \
REQUIRE_LIVE_APPLICATION_MYSQL_PROOF=true \
bash worklog/v1.0.0/sprint-33-202607/it/test_f2_application_mysql_authority_runtime_http.sh

PROVED_CASE_COUNT="$(printf '%s\n' $CASE_IDS | wc -l | tr -d ' ')"
PROVED_CASE_IDS="$(printf '%s\n' $CASE_IDS | paste -sd, -)"
...
echo "proved_case_ids=${PROVED_CASE_IDS}"
if [ "$PROVED_CASE_COUNT" = "1" ]; then
  echo "proved_case_id=${PROVED_CASE_IDS}"
fi
