...
PROOF_API_PATH="${COPILOT_FINANCE_PROOF_API_PATH:-/api/ai/finance/application-mysql-authority}"
...
normalize_case_ids() {
  if [ "$CASE_IDS_RAW" = "all" ]; then
    printf '%s' "$CASES_JSON" | jq -r '.data[].id'
    return 0
  fi
  printf '%s\n' "$CASE_IDS_RAW" | tr ',' ' '
}
...
UNAUTH_STATUS="$(http_status /tmp/codex-f2-proof-unauth.json "${BASE_URL}${PROOF_API_PATH}/cases")"
...
for CASE_ID in $CASE_IDS; do
  ...
  if [ -z "$PROVED_CASE_IDS" ]; then
    PROVED_CASE_IDS="$CASE_ID"
  else
    PROVED_CASE_IDS="${PROVED_CASE_IDS},${CASE_ID}"
  fi
  LAST_PROVE_STATUS="$PROVE_STATUS"
  LAST_RUN_STATUS="$RUN_STATUS"
  LAST_REPORT_COUNT="$REPORT_COUNT"
  LAST_RUN_MESSAGE="$RUN_MESSAGE"
  LAST_FIRST_FAILURE="$FIRST_FAILURE"
done
...
echo "prove_http_status=${LAST_PROVE_STATUS}"
echo "prove_run_status=${LAST_RUN_STATUS}"
echo "prove_report_count=${LAST_REPORT_COUNT}"
if [ -n "$LAST_RUN_MESSAGE" ]; then
  echo "prove_message=${LAST_RUN_MESSAGE}"
fi
if [ -n "$LAST_FIRST_FAILURE" ]; then
  echo "prove_failure=${LAST_FIRST_FAILURE}"
fi
