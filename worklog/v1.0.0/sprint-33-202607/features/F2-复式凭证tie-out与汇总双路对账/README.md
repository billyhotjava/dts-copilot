...
让 copilot 的财务汇总数据可被证明：一是双路独立计算对账（copilot vs 权威基准报表/黄金 SQL），二是锚到已过账凭证的复式（借=贷）账本——后者是会计级证明，不是"看起来对"。
...
- [x] 凭证账本（debit/credit + subjectId）可作为对账锚接入
- [ ] 核心汇总 copilot SUM 与权威基准端点/黄金 SQL 双路相等（到分）
