...
| F2 | 冷启动首屏 | 4 | P0 | READY |
| F3 | 对话脊柱(CopilotChat 拆分扶正) | 5 | P0 | READY |
...
F1 (骨架) ──┬─> F3 (对话脊柱) ──> F5 (乐观回答) ──> F6 (溯源)
            ...
            └─> F2 (首屏)                            v
                                              F7 (资产沉淀)
```
F1 是所有其他 Feature 的前置;F5 依赖 F3+F4;F6 依赖 F5;F7 依赖 F4+F5。

## 后台契约依赖(实施前必读,跨 F5/F6)
...
**容错原则**:前端一律「有则用、无则降级不报错」,后台补字段前不阻塞 UI 上线。后台契约扩展建议单列 Sprint(后台轨)或并入本 Sprint 增设 F8(后台契约),实施排期时确认。

## 完成标准

- [ ] 进入应用第一眼是 agent 工作台(冷启动首屏),不再是旧的 6 菜单
- [ ] 旧业务路由已删除,`/public/*` 三类分享链接仍可访问
- [ ] 一句话(文字或语音)问数能流式出结果,口径芯片常驻可改
- [ ] 结果可一键「存为卡片」「钉到看板」,资产库二级入口可浏览
- [ ] 点 SQL·溯源 能看到命中口径/表/字段/SQL
