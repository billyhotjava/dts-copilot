...
| D4 | 平台改动 | **指标业务零改码**:4 个端点 + DTO 字段均已就位;live 验证后仅补服务间认证只读白名单/配置 |
| D5 | 跨服务认证 | **试用期:单受限只读机器账号**(简单快部署),复用 `X-DTS-Service` / `X-DTS-Service-Token`。⚠️ 生产化/多租户前应切回**令牌透传**以恢复 Ranger 行列权限与指标密级控制——本期是显式权宜,非永久方案 |
...
**dts-platform 侧:** 指标业务零改码;仅补一个**受限只读服务身份**(治理指标读白名单/token 配置)。

## 7. 跨服务集成与健壮性

- **认证(D5)**:试用期用单受限只读机器账号,优先复用 dts-platform 既有服务认证头。Keycloak client-credentials 保留为后续令牌透传/细粒度权限方案选项。⚠️ 生产/多租户前切令牌透传恢复行列权限/密级。
